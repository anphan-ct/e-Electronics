  ...
  exports.getSecurityAlerts = async (req, res) => {
    try {
      ...
      // LẤY DANH SÁCH CÁC CẢNH BÁO ĐÃ TẮT TỪ DATABASE
      const dismissedRows = await query("SELECT alert_key FROM dismissed_alerts");
      const dismissedKeys = dismissedRows.map(r => r.alert_key);
      const isDismissed = (key) => dismissedKeys.includes(key);

      // Chuỗi ngày hiện tại (YYYY-MM-DD) để làm key (Sang ngày hôm sau cảnh báo mới sẽ hiện lại)
      ...
      const suspiciousIPs = await query(`
        ...
        FROM login_logs WHERE status = 'failed' AND created_at >= DATE_SUB(NOW(), INTERVAL 1 HOUR)
        GROUP BY ip_address HAVING fail_count >= 10 ORDER BY fail_count DESC LIMIT 10
      `);

      suspiciousIPs.forEach(row => {
        const alertKey = `suspicious_ip_${row.ip_address}_${todayStr}`;
        if (!isDismissed(alertKey)) {
          alerts.push({
            id:       alertKey, // Thêm id để FE gọi API tắt
            type:     "suspicious_ip",
      ...
      const lockedAccounts = await query(`
        ...
        FROM users u WHERE u.locked_until > NOW() ORDER BY u.login_attempts DESC LIMIT 20
      `);

      lockedAccounts.forEach(u => {
        const alertKey = `locked_account_${u.id}_${new Date(u.locked_until).getTime()}`;
        if (!isDismissed(alertKey)) {
          alerts.push({
            id:       alertKey,
            type:     "locked_account",
      ...
      const multiIPEmails = await query(`
        SELECT email, COUNT(DISTINCT ip_address) as ip_count, COUNT(*) as total_fails, MAX(created_at) as last_attempt
        FROM login_logs WHERE status = 'failed' AND created_at >= DATE_SUB(NOW(), INTERVAL 1 HOUR) AND email IS NOT NULL
        GROUP BY email HAVING ip_count >= 3 ORDER BY ip_count DESC LIMIT 10
      `);

      multiIPEmails.forEach(row => {
        const alertKey = `multi_ip_${row.email}_${todayStr}`;
        if (!isDismissed(alertKey)) {
          alerts.push({
            id:       alertKey,
      ...
      // ── Rule 4: Tỉ lệ thất bại cao toàn hệ thống ────────
      const [recentStats] = await query(`
        SELECT COUNT(*) as total, SUM(status = 'failed') as failed, SUM(status = 'success') as success
        FROM login_logs WHERE created_at >= DATE_SUB(NOW(), INTERVAL 30 MINUTE)
      `);
      ...
      if (recentStats.total > 10) {
        ...
        if (failRate >= 70) {
          ...
          const alertKey = `high_fail_rate_${todayStr}_${currentHour}`;
          if (!isDismissed(alertKey)) {
            alerts.push({
              id:       alertKey,
              type:     "high_fail_rate",
      ...
      // ── Rule 5: Admin đăng nhập thành công ──────────────
      const adminLogins = await query(`
        SELECT ll.email, ll.ip_address, ll.created_at FROM login_logs ll JOIN users u ON ll.user_id = u.id
        WHERE ll.status = 'success' AND u.role = 'admin' AND ll.created_at >= DATE_SUB(NOW(), INTERVAL 1 HOUR)
        ORDER BY ll.created_at DESC LIMIT 5
      `);

      adminLogins.forEach(row => {
        const alertKey = `admin_login_${row.email}_${new Date(row.created_at).getTime()}`;
        if (!isDismissed(alertKey)) {
          alerts.push({
            id:       alertKey,
            type:     "admin_login",
...
// ════════════════════════════════════════════════════════════
// POST /api/security/alerts/dismiss
// Admin tắt một cảnh báo thủ công
// ════════════════════════════════════════════════════════════
exports.dismissAlert = async (req, res) => {
  const { alertKey } = req.body;
  if (!alertKey) return res.status(400).json({ message: "Thiếu mã cảnh báo (alertKey)" });

  try {
    // INSERT IGNORE giúp không báo lỗi nếu admin vô tình bấm 2 lần
    await query("INSERT IGNORE INTO dismissed_alerts (alert_key) VALUES (?)", [alertKey]);
    res.json({ message: "Đã ẩn cảnh báo" });
  } catch (err) {
    res.status(500).json({ message: "Lỗi server", error: err.message });
  }
};