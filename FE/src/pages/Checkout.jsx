...
// ĐÃ TÍCH HỢP HỆ THỐNG ĐIỂM TÍCH LŨY
// Thay thế hoàn toàn file Checkout.jsx cũ
...
import {
  ...
  ChevronRight, CheckCircle2, Circle, Loader2, Lock
} from "lucide-react";
import "../Checkout.css";
...
function Checkout() {
  ...
  const discount = loyaltyRedeem.discountUsd || 0;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (paymentMethod === "cod") {
        // COD: Tạo đơn thẳng, tích điểm ngay
        ...
        // VNPay: Tạo URL thanh toán, truyền pointsToUse
        const res = await axios.post(
          "http://localhost:5000/api/vnpay/create-payment",
          {
            userId: user.id,
            amount: subtotal, // Gửi subtotal gốc, BE tự tính discount
            orderInfo: `Thanh toan ${cart.length} san pham MyShop`,
            items: cart.map((item) => ({ id: item.id, quantity: item.quantity || 1, price: item.price })),
            shippingName: formData.name,
            shippingPhone: formData.phone,
            shippingAddress: formData.address,
            pointsToUse: loyaltyRedeem.pointsToUse, // ← THÊM MỚI
          },
          { headers: { Authorization: `Bearer ${token}` } }
        );

        if (loyaltyRedeem.pointsToUse > 0) {
          clearCart(); // Clear cart trước khi redirect VNPay
        }
